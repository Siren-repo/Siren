...
public class UserControllerTest {
  ...
  @MockBean private JwtTokenUtils utils;
  ...
  void registerWithBlankString() throws Exception {
    UserRegisterRequest request = UserRegisterRequest.builder().email("").build();
    mockMvc
        .perform(
            post("/api/users")
                .with(csrf())
  ...
  void logout() throws Exception {
    mockMvc
        .perform(patch("/api/users/sessions").with(csrf()).contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andDo(print())
        .andReturn();
  }
  ...
  void reissue() throws Exception {
    ...
    mockMvc
        .perform(
            patch("/api/users/sessions/renew")
                .with(csrf())
                .contentType(MediaType.APPLICATION_JSON)
  ...
  void reissueWithNotAuthHeader() throws Exception {
    mockMvc
        .perform(
            patch("/api/users/sessions/renew").with(csrf()).contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isUnauthorized())
        .andDo(print());
  }
}

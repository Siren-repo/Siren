...
public class CategoryController {
    ...
    public ApiResponse<CategoryResponse> createCategory(@RequestBody @Valid CategoryCreateRequest categoryCreateRequest, @AuthenticationPrincipal UserDetailsDto user) {
        return ApiResponse.ok(ResponseCode.Normal.CREATE, categoryService.register(categoryCreateRequest, user));
    }

    @GetMapping
    public ApiResponse<Page<CategoryResponse>> findCategoriesByCategoryType(@NotBlank @RequestParam("categoryType") String categoryType,
                                                          @PageableDefault(size = 5, sort = "categoryId", direction = Sort.Direction.DESC) Pageable pageable) {
        return ApiResponse.ok(ResponseCode.Normal.RETRIEVE, categoryService.findAllByType(CategoryType.of(categoryType), pageable));
    }
